...
//adding object elements
    id: "item001",
    inCart: false //element not used for all
  },
  {
    name: "Fringed Plaid",
    "price": 18.99,
...
function addToCart(item){
  ...
  for (var i = 0; i<products.length; i++){
    if(products[i].id === item){
      ...
      var qty = document.getElementById("cartCount");
...
function removeFromCart(item){
  ...
  for(var i = 0; i<cart.length;i++){
    if(cart[i].id === item){
      cart.splice(i,1);
      // cart.pop(cart[i]);doesn't work for removing the correct item in all scenarios
      //console.log("remove " + cart[i].id ); testing to see if item of interest is removed
      var qty = document.getElementById("cartCount");
      qty.innerHTML = " " + cart.length;
      break;
    }
